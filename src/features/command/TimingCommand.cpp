...
#include "features/DefaultEntitySystemsCollection.h"
...
void registerTimingCommand() {
    ...
    cmd.overload<ECSTiming>().optional("counts").execute([logger](
                                                             CommandOrigin const&,
                                                             CommandOutput&,
                                                             ECSTiming const& param
                                                         ) {
        bool expected = false;
        ...
        auto  thread     = std::thread([logger, showCounts]() mutable {
                ...
                sleeper.sleepFor(1_tick);
                {
                    ...
                    for (auto& collectCategory : collection.mTickingSystemCategories) {
                        ...
                        for (size_t j = 0; j < tickTimings.size(); j++) {
                            ...
                            timing.mTime  += tickTimings.at(j).mTime;
                        }
                    }
                }
            }
            ...
            std::chrono::duration<double, std::milli> allTime;
            for (auto& [systemId, timing] : timings) {
                orderdTiming.emplace_back(systemId, timing.mTime / counttick, timing.mCount);
                allTime += timing.mTime / counttick;
            }
            ...
            logger->warn("{} ECS cost {:.5f}ms per tick (show top {}):", timings.size(), allTime.count(), showCounts);
